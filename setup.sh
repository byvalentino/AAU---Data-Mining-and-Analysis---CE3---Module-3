...
python3 -c "import pandas, numpy, pyarrow, sklearn, plotly, mlflow" || {
  echo "setup failed: the requirements did not install —"
  echo "importing pandas, numpy, pyarrow, sklearn, plotly, mlflow failed. Read pip's message above."
  exit 1; }
...
echo "Four labs are in labs/, with two to four functions each, marked TODO."
echo "prepare.py also wrote the local MLflow store (mlruns.db and mlartifacts/):"
echo "one run per model, with its parameters, metrics, environment and signature,"
echo "and the aliases champion and challenger. No server, no network."
