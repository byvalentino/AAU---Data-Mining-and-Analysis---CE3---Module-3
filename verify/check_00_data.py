...
def main() -> int:
    ...
    for entry in manifest.get("files", []):
        ...
        if entry.get("kind") == "table":
            ...
            # The MLflow artefact root: run identifiers inside it are random, so
            # the manifest records how many files setup wrote, not their names.
            ...
            if entry.get("files") and present != entry["files"]:
                return not_ready(
                    f"{entry['path']}/ holds {present} file(s); setup wrote {entry['files']}. "
                    "Something rewrote the MLflow store; re-run setup.")
